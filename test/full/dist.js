(function (global, factory) {
  ...
  function createPatchFunction(backend) {
      ...
      function removeAndInvokeRemoveHook(vnode, rm) {
          if (isDef(rm) || isDef(vnode.data)) {
              ...
              if (isDef(i_4 = vnode.componentInstance) && isDef(i_4 = i_4.VNode) && isDef(i_4.data)) {
      ...
      // 为初始化的插入的 queue 父data打标记 pendingInsert
  ...
  // 只有组件并且 Ctor.options.inheritAttrs!==false 才进行该 update 
  ...
  // el.style.setProperty 的兼容性设置
  var setProp = function (el, name, val) {
      /* istanbul ignore if */
      if (cssVarRE.test(name)) {
          el.style.setProperty(name, val);
          // 处理style 中的 important 
      }
  ...
  // 以兼容的方式处理静、动态 style
  // 并会对处理过的 styleData 存储到 vnode.data.normalizedStyle
